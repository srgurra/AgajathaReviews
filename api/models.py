...
class Movie(models.Model):
    ...
    postercard_url = models.CharField(max_length=255)
...
class WebSeries(models.Model):
    ...
    def save(self, *args, **kwargs):
        existing_web_series = WebSeries.objects.filter(title=self.title).first()
        if existing_web_series:
            self.id = existing_web_series.id
            self.created_at = existing_web_series.created_at
        self.updated_at = timezone.now()
        super().save(*args, **kwargs)
...
class MoviesReview(models.Model):
    ...
    poster = models.CharField(max_length=100, db_collation='SQL_Latin1_General_CP1_CI_AS')
